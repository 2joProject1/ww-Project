...
<body>
	...
	<div id="container">
		...
		<div id="content-layout">
			<div class="container">
				...
				<div class="row">
					<div class="row dt_margin" style="margin-bottom: 10px">
						<div class="col">
				  ...
				  <path fill-rule="evenodd"
						...
						<span class="dt_name">익명</span>
						 <span> <fmt:formatDate
								value="${detail.enrollDate }" pattern="MM/dd hh:mm" />
						</span>
					</div>
				...
				<div class="row dt_margin">
					<div class="col">${detail.boardContent }</div>
				</div>
				<br>
				<div class="row dt_margin">
					<c:forEach var="a" items="${attachment }">
						<div class="col-4 imgCol">
							<img alt="이미지" src="resources/coFile/${a.fileName }"
								class="fileImg" width="100%">
						</div>
					</c:forEach>
				</div>
				<div class="row dt_margin">
				...
				<div class="row dt_margin dt_last">
					<div class="col-1" onclick="thumbClick()">
						<p class="dt_thumbs" style="margin-top: 30px">
							<span id="thumb_before"> <svg
						  ...
						  <path
							...
							</span> <span id="showGood"> ${detail.communityGood } </span>

						</p>
					</div>
					<div class="col-1">
						<p class="dt_chat" style="margin-top: 30px">
					...
					<div class="col-10 dt_btns"
						...
						<c:if test="${loginUser.memberNo == detail.boardWriter}">
							<button
								onclick="location.href='modifyPage.co?pageNo=${detail.boardNo}'"
								id="cm_detail_btn">수정</button>
							<button
								onclick="location.href='delete.co?pageNo=${detail.boardNo}'"
								id="cm_detail_btn">삭제</button>
						</c:if>
				...
				<c:forEach items="${replys}" var="r">
					<form id="CommunityDeleteForm" method="post"
						action="deleteReply.co" style="margin-top: 6px;">
					  ...
					  <path fill-rule="evenodd"
								...
								<b>익명</b>
								<span class="col-12 reply_margin">
							...
							<div class="col-1">
								<c:if test="${loginUser.memberNo == r.memberNo}">
									<button type="submit" class="replyDeleteBtn"
										style="float: right;"><b>X</b></button>
								</c:if>
							</div>

							<div class="col-12 reply_margin">${r.replyContent }</div>
							<br>
				...
				<div class="row replyDiv2">
					<div class="col-12">
						<form action="insertReply.co" method="post"
							id="communityInsertForm">
							<input type="text" class="replyInput"
								placeholder="댓글을 입력해주세요 30자이내" name="replyContent"
								required="required" /> <img src="resources/images/send.svg"
								class="sendImg" id="sendImg" onclick="onclickImg()"> <input
								type="hidden" name="boardNo" value="${detail.boardNo }" />
						</form>
					</div>
				</div>
				<input type="hidden" name="communityGood"
					value="${detail.communityGood}" id="d_communityGood" /> <input
					type="hidden" name="boardNo" value="${detail.boardNo}"
					id="d_boardNo" /> <input type="hidden" id="loginUserInfo"
	...
	<script type="text/javascript">
		...
		function thumbClick() {
			...
			var communityGood;
			if (minCg) {
				communityGood = $('#d_communityGood').val() - 1;
			} else {
				communityGood = $('#d_communityGood').val();
			}
			var nowMember = $('#loginUserInfo').val();
			var param = {
				'boardNo' : boardNo,
				'communityGood' : communityGood,
				'boardWriter' : nowMember
			};
			if (isClick == false) {
